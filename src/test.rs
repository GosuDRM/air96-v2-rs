...
fn matrix_sym_defer_pk_debouncing() {
    use crate::keyboard::matrix::Matrix;

    let mut m = Matrix::new();
    
    // Simulate raw state differs from debounced state on key (row 1, col 2).
    // debounced state is 0 (released).
    // Let's set counters[1][2] to 10, as if scan() detected a change to pressed (true).
    m.counters[1][2] = 5;

    // Call tick_debounce 4 times -> counter should decrement, but no event generated yet
    for i in (2..=5).rev() {
        assert_eq!(m.counters[1][2], i);
        m.tick_debounce();
        assert!(m.pending_events.is_empty());
    }
    
    // Counter is now 1
    assert_eq!(m.counters[1][2], 1);

    // Call tick_debounce 5th time -> counter becomes 0, event should be generated
    m.tick_debounce();
    assert_eq!(m.counters[1][2], 0);

    // pending_events should now contain the press event
    ...
    // Drain events to simulate scan() draining them
    m.pending_events.clear();

    // Debounced bit for (1, 2) should now be 1 (pressed)
    let bit_pos = 1 * 21 + 2;
    assert_ne!(m.debounced[bit_pos / 8] & (1 << (bit_pos % 8)), 0);

    // Now simulate release (debounced is 1 (pressed), raw state becomes 0 (released)).
    // Let's set counters[1][2] to 5.
    m.counters[1][2] = 5;

    // Call tick_debounce 4 times -> no event yet
    for _ in 0..4 {
        m.tick_debounce();
    }
    assert_eq!(m.counters[1][2], 1);
    ...
    // Call tick_debounce 5th time -> counter becomes 0, event generated
    m.tick_debounce();
    assert_eq!(m.counters[1][2], 0);

    // pending_events should now contain the release event
    assert_eq!(m.pending_events.len(), 1);
    let ev = m.pending_events[0];
    assert_eq!(ev.row, 1);
    assert_eq!(ev.col, 2);
    ...
    // Debounced bit for (1, 2) should now be 0 (released)
    assert_eq!(m.debounced[bit_pos / 8] & (1 << (bit_pos % 8)), 0);
}
