...
Fixed Windows USB device not recognized - stale descriptor cache + missing IAD.

### Fixed

- **Windows still rejected the device after v4.7.9.** Two root causes:
  1. **Stale Windows USB descriptor cache.** The iSerial string was hardcoded as
     v4.7.4 in usb_hid.rs and never bumped through v4.7.5-v4.7.9. Windows
     caches USB device descriptors indexed by VID+PID+iSerial, so it served the
     old broken (pre-v4.7.9) descriptors even after reflashing. The serial number
     now auto-derives from CARGO_PKG_VERSION, so every release automatically
     invalidates the cache.
  2. **No IAD for 4-interface composite device.** The device descriptor used
     device class 0x00 (use interface descriptors) with 4 HID interfaces but
     no Interface Association Descriptor. Windows xHCI driver can reject
     multi-function devices that lack IAD. Changed to device class 0xEF
     (Miscellaneous), sub class 0x02 (Common Class), protocol 0x01 (IAD) -
     the standard declaration for composite HID keyboards. Linux and
     macOS are unaffected (they never required IAD).
...
- **Use dfu-util to flash, not QMK Toolbox.** QMK Toolbox does not reliably
  ...
  then: dfu-util -d 0483:DF11 -a 0 -s 0x08000000:leave -D air96-v2-4.8.1.bin

### Files

- src/usb_hid.rs - serial number from CARGO_PKG_VERSION; device class 0xEF/0x02/0x01 (IAD)
- Cargo.toml / README.md / CHANGELOG.md - bumped to 4.8.1

## v4.7.9 (2026-06-09)

Fix Windows "USB device not recognized" — malformed System Control and Consumer HID descriptors.

### Fixed

- **Windows refused to enumerate the keyboard ("USB device not recognized"); Linux was fine.** The System Control interface (`SystemControlReport16`) was generated by `#[gen_hid_descriptor]` from a `u16` field, which produced a broken **array** report descriptor: `LOGICAL_MAXIMUM` was derived from the field type as **65535** even though the array's usage range is only `0x81..0xB7`, and the macro's spec-level `logical_min` injected a stray reserved Main item (`0x11 0x01`, an undefined Main tag). Linux's lenient HID parser tolerates both; Windows' strict `hidparse.sys` rejects the interface, surfacing as "USB device not recognized." Replaced it with a hand-written `SYSTEM_DESC` (like `BOOT_KEYBOARD_DESC`/`VIA_DESC`) that matches QMK's C descriptor (`usb_descriptor.c:300-310`): `USAGE_MINIMUM 0x01`, `USAGE_MAXIMUM 0xB7`, `LOGICAL_MINIMUM 1`, `LOGICAL_MAXIMUM 0xB7`, 16-bit array, no Report ID. Verified the rebuilt binary's descriptor is byte-identical to the C reference. The USB clock (HSI48 + CRS) and the v4.7.6 enumeration-window fix were ruled out as unaffected.
- **`MAC_DND` sent the wrong System Control usage on USB.** The USB path sent `0x9B - 0x81 + 1 = 27` (a 1-based index against the old descriptor's bogus bounds), selecting an undefined usage even on Linux. With the corrected descriptor (value→usage 1:1) it now sends the raw usage `0x9B`, matching the wireless path and the C firmware.
- **The Consumer (media-key) interface had the identical descriptor defect.** It was still built from usbd-hid's `MediaKeyboardReport`, a `u16` array whose `#[gen_hid_descriptor]` emits the same broken `LOGICAL_MAXIMUM = 65535` (`0x27 0xFF 0xFF 0x00 0x00`, derived from the field *type*) over a usage range of only `0x00..0x0514` — so Windows rejected the whole composite device for this interface too (Linux fine). Replaced `MediaKeyboardReport::desc()` with a hand-written `CONSUMER_DESC` with `LOGICAL_MAXIMUM = USAGE_MAXIMUM = 0x0514`, so reported values map 1:1 to usages (0 releases). With this, all four HID report descriptors (Boot, VIA, System, Consumer) validate clean — Consumer was the last `#[gen_hid_descriptor]`-generated one on the wire.

### Files

- `src/usb_hid.rs` — replaced the `#[gen_hid_descriptor]` `SystemControlReport16` and the Consumer interface's `MediaKeyboardReport` with hand-written `SYSTEM_DESC` and `CONSUMER_DESC`; `send_system`/`send_consumer` push a 2-byte little-endian selector via `push_raw_input`; dropped the now-unused `generator_prelude` / `MediaKeyboardReport` / `SerializedDescriptor` imports
- `src/main.rs` — `MAC_DND` USB path sends the raw usage `0x9B`
- `Cargo.toml` / `README.md` / `CHANGELOG.md` — bumped to 4.7.9
