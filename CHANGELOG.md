...
Fix Windows "USB device not recognized" — malformed System Control HID descriptor.
...
- `src/usb_hid.rs` — replaced the `#[gen_hid_descriptor]` `SystemControlReport16` with the hand-written `SYSTEM_DESC`; `send_system` now pushes a 2-byte little-endian selector via `push_raw_input`; dropped the now-unused `generator_prelude` import
